...
describe('Testing DELETE /v1/admin/quiz/trash/empty', () => {
  ...
  test('Quiz not in trash', () => {
    ...
    const create = v2adminQuizCreate(token, 'quiz2', 'lorem ipsum');
  ...
  test('Quiz does not belong to user, but is in trash', () => {
    ...
    const create = v2adminQuizCreate(newUser.token, 'quiz2', 'lorem ipsum');

    v2adminQuizRemove(newUser.token, create.quizId);
